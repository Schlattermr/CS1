...
int *randomArray(int n) {
  ...
  for(int i=0; i<n; i++) { //declares i in for-loop instead
    a[i] = random() % 10;
  }
  return a; //returns created array
}

double average(const int *a, int n) {
  int sum = 0; //initialized sum and moved declaration of i to for-loop
  for(int i=0; i<n; i++) { //changed N to n
    sum += a[i];
  }
  return ((double) sum / n); //added parenthesis
}
