...
void printBoard(Move **board) {
  ...
  for(int i=0; i<3; i++) {
    ...
    printf("\n");
    if(i<2) { // changed to 2 so there is no unnecessary line at the bottom of the board
      printf("  ------+-------+------\n");
    }
  }
  printf("\n");
...
Status getStatus(Move **board) {
  ...
  if( //rows
      (board[0][0] == board[0][1] && board[0][1] == board[0][2] && board[0][0] == X) || //changed so conditions aren't always true by checking the same array index
    ...
    for(i=0; i<3; i++) {
      for(j=0; j<3; j++) {
        if(board[i][j] == NONE) { //checks to see if there is any empty space still
          s = PLAYING;
        }
      }
    }
