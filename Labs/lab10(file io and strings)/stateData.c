...
void toXMLFile(char **states, int *populations, int numStates) {
  ...
  FILE *outstream = fopen("stateData.xml", "w");

  fprintf(outstream, "<STATES>\n");
  for(int i=0; i<numStates; i++) {
    fprintf(outstream, " <STATE>\n");
    rtrim(states[i]);
    fprintf(outstream, "   <NAME>%s</NAME>\n", states[i]);
    fprintf(outstream, "   <POPULATION>%d</POPULATION>\n", populations[i]);
  }
  fclose(outstream);
